...
function titleForView() {
  ...
  return {
    all: '全部任务', today: '今日待办', overdue: '逾期任务', upcoming: '未来 7 天',
...
function taskHtml(task) {
  ...
  return `<article class="task-card ${overdue ? 'overdue' : ''} ${state.selected.has(task.id) ? 'selected' : ''} ${task.color ? 'custom-color' : ''}" style="${task.color ? `--task-color:${esc(task.color)}` : ''}" draggable="true" data-id="${task.id}">
    <button class="check ${task.completed ? 'done' : ''}" data-action="toggle">${task.completed ? '✓' : ''}</button>
    <div>
      ...
      <div class="meta">
        ${task.dueAt ? `<span class="pill ${overdue ? 'high' : ''}">🕒 ${fmtDate(task.dueAt)}</span>` : ''}
...
function openTaskModal(id = null) {
  ...
  const task = existing || {
    ...
    dueAt: null, reminders: [30], repeat: null, subtasks: [], attachments: []
  };
  let draftAttachments = structuredClone(task.attachments || []);
  const localDue = task.dueAt
    ? new Date(new Date(task.dueAt).getTime() - new Date().getTimezoneOffset() * 60_000).toISOString().slice(0, 16)
    : '';
  ...
  modal(`<h2>${existing ? '编辑待办' : '新建待办'}</h2>
    <div class="form-grid">
      ...
      <div class="field"><label>优先级</label><select id="fPriority"><option value="high" ${task.priority === 'high' ? 'selected' : ''}>高</option><option value="medium" ${task.priority === 'medium' ? 'selected' : ''}>中</option><option value="low" ${task.priority === 'low' ? 'selected' : ''}>低</option></select></div><div class="field"><label>任务颜色</label><div class="color-field"><input id="fColor" type="color" value="${task.color || '#5b7cfa'}"><label><input id="fUseColor" type="checkbox" ${task.color ? 'checked' : ''}> 启用自定义颜色</label></div></div>
      <div class="field"><label>截止时间</label><input id="fDue" type="datetime-local" value="${localDue}"></div>
  ...
  $('#saveTask').onclick = async () => {
    ...
    const object = {
      ...
      dueAt: $('#fDue').value ? new Date($('#fDue').value).toISOString() : null,
